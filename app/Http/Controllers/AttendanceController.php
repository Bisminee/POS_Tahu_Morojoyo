...
class AttendanceController extends Controller
{
    public function index()
    {
        ...
        if ($activeAttendances->count() > 0) {
            $firstAttendance = $activeAttendances->first();
    ...
    public function exportAbsensiCsv(Request $request)
    {
        ...
        $callback = function () use ($attendances) {
            ...
            foreach ($attendances as $attendance) {
                fputcsv($file, [
                    optional($attendance->tanggal)->format('d-m-Y') ?? $attendance->tanggal ?? '-',
                    $attendance->karyawan?->nama ?? '-',
                    $attendance->cabang?->namaCabang ?? '-',
                    optional($attendance->jam_masuk)->format('H:i:s') ?? $attendance->jam_masuk ?? '-',
                    optional($attendance->jam_pulang)->format('H:i:s') ?? $attendance->jam_pulang ?? '-',
                    $attendance->status ?? '-',
